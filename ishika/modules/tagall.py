...
async def tag_all(client, message: Message):
    ...
    for member in members:
        ...
        if member.user.is_bot or member.user.deleted:
            continue
        ...
        try:
            ...
            await client.send_message(chat_id, f"{mention} {mention_text}")
            await asyncio.sleep(3) # 2 se 3 sec kiya spam se bachne ke liye

            if count % 5 == 0:
                await asyncio.sleep(7) # 5 sec se 7 sec
...
async def tag_admins(client, message: Message):
    admins = [admin.user.mention async for admin in client.get_chat_members(message.chat.id, filter=enums.ChatMembersFilter.ADMINISTRATORS)]
